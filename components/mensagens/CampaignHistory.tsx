...
const ACTIVE = new Set<WhatsAppCampaign['status']>(['SCHEDULED', 'QUEUED', 'SENDING', 'PAUSED', 'CANCELING']);
...
export function CampaignHistory({ api }: { api: WhatsAppApi }) {
  ...
  }} retryAlreadyExists={retriedCampaignIds.has(selected)} onRetry={async (campaignId, idempotencyKey) => {
    const retry = await api.retryFailedRecipients(campaignId, idempotencyKey);
  ...
  return <section className="space-y-5"><div className="flex items-center justify-between"><h2 className="text-xl font-black">Histórico de campanhas</h2>
    <button onClick={() => void api.syncCampaigns().then(load).catch((caught) => {
      setError(caught instanceof Error ? caught.message : 'Erro ao sincronizar.');
    })} className="rounded-xl border px-4 py-2 font-bold">Sincronizar ativas</button></div>
